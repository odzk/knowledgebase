...
/** GET /api/admin/articles — list all articles with category info */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO nuvho_kb.articles
           (slug, title, description, category_slug, content, read_time, featured, sort_order, updated_at)
    ...
    return NextResponse.json({ success: true, slug, categorySlug: categorySlug.trim() })
  } catch (err: any) {
    if (err.code === '23505') {
